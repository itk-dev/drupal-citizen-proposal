...
final class OpenIDConnectController implements ContainerInjectionInterface {
   ...
   */
  private const SESSION_LOGIN_LOCATION = 'citizen_proposal_openid_connect.login_location';

  /**
   * Session name for storing OAuth2 state.
   */
  private const SESSION_STATE = 'citizen_proposal_openid_connect.oauth2state';

  /**
   * Session name for storing OAuth2 nonce.
   */
  private const SESSION_NONCE = 'citizen_proposal_openid_connect.oauth2nonce';

  /**
   * Session name for storing is token.
   */
  private const SESSION_ID_TOKEN = 'citizen_proposal_openid_connect.id_token';
  ...
  public static function create(ContainerInterface $container): self {
    return new static(
      $container->get('citizen_proposal.authentication_helper'),
      $container->get('session'),
      $container->get('drupal_psr6_cache.cache_item_pool'),
      $container->get('language_manager'),
      $container->get('renderer'),
      $container->get('event_dispatcher'),
      $container->get('logger.channel.citizen_proposal_openid_connect')
    );
  }
  ...
  public function endSession(Request $request): Response {
    ...
    $endSessionUrl = $provider->getEndSessionUrl($postLogoutRedirectUri, NULL, $this->getSessionValue(self::SESSION_ID_TOKEN));
  ...
  private function process(Request $request): Response {
    ...
    if ($accessDeniedLocation = $event->getAccessDeniedLocation()) {
      ...
      return new LocalRedirectResponse(Url::fromRoute(
        'citizen_proposal_openid_connect.openid_connect_end_session',
        [
